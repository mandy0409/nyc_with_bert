...
class SegmentEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        segment_title = torch.tensor(0).repeat(title_end_ix + 1).to(self.device) # Include CLS, SEP token
        segment_body = torch.tensor(1).repeat(body_end_ix - title_end_ix).to(self.device) # It's already include SEP token because ix is index
        segment_ans = torch.tensor(2).repeat(x.size(1) - (body_end_ix + 1)).to(self.device) # Add 1 becaues it's index
